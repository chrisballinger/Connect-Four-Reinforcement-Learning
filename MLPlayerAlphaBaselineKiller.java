...
public class MLPlayerAlphaBaselineKiller
{
	...
	// Constructor.
	public MLPlayerAlphaBaselineKiller(Player p)
	{
		thePlayer=p;
	}
...
//for (int i = 0; i < features.size(); i++) System.out.println("(" + features.get(i).start.r + ", " + features.get(i).start.c + ")\t" + features.get(i).theta + "\t" + features.get(i).length + "\t" + features.get(i).type);
			...
			if(isFirstPlayer)
			{
				switch(moveCount)
				{
					case 1: 	mess.move = 7; break;
					case 2: 	mess.move = 9; break;
					case 3: 	mess.move = 1; break;
					case 4:		mess.move = 8; break;	
					case 5:		mess.move = 10; break;	
					default: 	mess.move = selected_column; break;
				}
			}
			else
	...
	public static void main(String[] args)
	{
		...
		MLPlayerAlphaBaselineKiller me = new MLPlayerAlphaBaselineKiller(p);
		try
		{
			me.play(my_id);
		}
